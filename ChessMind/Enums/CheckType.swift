...
enum CheckState {
  ...
  /// The king is in check from one enemy piece.
  /// In this scenario, the king can get out of
  /// the check in 3 ways: moving to an unattacked
  /// square, capturing the checking piece (if
  /// close enough) or blocking with an own piece
  /// (if there is at least one square between
  /// the piece attacking the king and the king)
